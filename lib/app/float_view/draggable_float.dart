...
class _CustomDraggableFloatState extends State<DraggableFloatWidget>
  ...
  _initBorderSize() {
    ...
    var mediaQueryData = MediaQueryData.fromView(window);
    ...
    _statusBarHeight = mediaQueryData.padding.top; // SafeArea top
    _bottomBarHeight = mediaQueryData.padding.bottom; // SafeArea bottom
  ...
  _initAnimation() {
    ...
    animation.addStatusListener((status) {
      ...
      OperateEvent event = receivedEventList.last;
      ...
      var tempNextState = event == OperateEvent.OPERATE_HIDE
          ? DraggableFloatWidgetState.HIDE
          : DraggableFloatWidgetState.SHOW;
      if (tempAnimEndState == tempNextState) return;
  ...
  _handleAnimValueChange() {
    ...
    double tempAbsX = (animStartPx - animEndPx).abs();
    double tempAbsY = (animStartPy - animEndPy).abs();
    ...
    if (currentState == DraggableFloatWidgetState.AUTO_ATTACH_IN_PROGRESS) {
      double tempX = animStartPx < animEndPx
          ? animStartPx + tempAbsX * value
          : animEndPx + tempAbsX * (1 - value);
      double tempY = animEndPy;
      if (animStartPy < _top) {
        tempY = animStartPy + tempAbsY * value;
      } else if (animStartPy > _bottom) {
        tempY = animEndPy + tempAbsY * (1 - value);
      }
      positionX = tempX;
      positionY = tempY;
      _safeSetState();
  ...
  Widget build(BuildContext context) {
    ...
    var dragWidget = _buildDragWidget();
  ...
  _handleDragEnd(Offset offset) {
    // 0. Assign the position at which the animation begins.
    animStartPx = offset.dx;
    animStartPy = offset.dy - (widget.config.isFullScreen ? 0 : _variableTop);
    // 1. Calculate the position at the end of the animation.
    animEndPx = offset.dx + widget.width / 2 <= _halfScreenW ? _left : _right;
    if (animStartPy <= _top) {
      animEndPy = _top;
